...
if (isset($_POST['saveChanges'])) {
    ...
    if (empty($name) || empty($email)) {
        ...
        // $getResult = mysqli_query($connect,$getUsersQuery);
        ...
        if($found){
            ...
            // $saveChanges = mysqli_query($connect, $editQuery);
            if($saveChanges){
                $_SESSION['message']= "Name is edited but Email is not edited, since it is already existed";
                header("location: dashboard.php");
            }
            else{
    ...
    if(in_array($ext2, $allowed_extension)){
       if($file_size < 10000000){
        ...
        if($upload){
           ...
           $_SESSION['errorMessage'] = "file size is more than 10mb";
           header("location: dashboard.php");
        }
    }
    else{
